...
#Тесты: https://github.com/selfedu-rus/test-python-base/tree/main/10/10.5.5
#Sample Input:
...
book = [t[i](x) if t[i] != str else x.strip() for i, x in enumerate(input().split(","))]
match book:
    case (_, str()  as author, str()  as name) if len(author) > 5 and len(name) > 9: print("Yes")
    case (_, str() as author, str() as name, float() as price) if len(author) > 5 and price > 0: print("Yes")
    case (_, str() as author, str() as name, int() as year) if year > 2019: print("Yes")
    case (_, str() as author, str() as name, float() as price, int() as year) if price > 0 and year > 2019:
        print("Yes")
    case book: print("No") 
